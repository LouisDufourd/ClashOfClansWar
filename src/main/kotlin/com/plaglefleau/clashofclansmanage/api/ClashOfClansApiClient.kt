...
interface ClashOfClansApiClient {
    ...
    suspend fun getClanMembers(@Path("clanTag") clanTag: String): ClanMembers

    @POST("players/{playerTag}/verifytoken")
    suspend fun verifyToken(@Path("playerTag") playerTag: String, @Body token: VerifyTokenRequest): VerifyTokenResponse
}